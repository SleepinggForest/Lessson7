...
function spawnApple() {
  ...
  // check that the apple did not hit the tail
  ...
  // checking that the apple got into the area and not around the edges somewhere
...
function spawnFire() {
  ...
  // check that the fire did not hit the tail
  for (let i = 0; i < tailSize-1; i++) {
...
function loop() {
  ...
  // checking that you crashed into yourself
  ...
  // check for snake head collisions with fire
  for (let a = 0; a < fires.length; a++) {
    if (
      px < (fires[a].x + fw)
      && px + pw > fires[a].x
